...
设置时，软件应通过IA32\_VMX\_ENTRY\_CTLS或IA32\_VMX\_TRUE\_ENTRY\_CTLS寄存器来检查当前VMX支持哪个功能，并且要确保保留位的设置时适当的。
...
这是，"entry to SMM"设为0值，表明需要返回到非SMM模式，从而VMM履行了一次对SMI的监控智能。
...
在一般的应用中，我们可以VM-entry MSR-load地址应该等于VM-exit MSR-store地址，这样保证它们在VM-exit于VM-exit时使用同一份MSR列表。
...
- bits[7:0]设置中断或异常的向量号，当时间类型是NMI时，向量号必须为2。事件类型为other时，向量号必须为0值。
