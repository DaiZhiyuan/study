...
```asm
    ...
    cpuid
```
如上所示，0B号功能就是mian leaf，EXC提供的0号就是sub-leaf。
...
```asm
    ...
    cpuid               ; 查询0号信息
```
返回的相应信息放在eax、ebx、ecx以及edx寄存器种。这些信息是32位的，因此在64位模式下，rax、rbx、rcx、rdx寄存器的高32位被清0。
...
如果是FF字节表示CPUID.EAX=02H不反悔cache及TLB信息，需要执行CPUID.EAX=04H进行查询。
